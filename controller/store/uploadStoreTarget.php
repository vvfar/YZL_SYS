            ...
            if($fileinfo['size']<120971520 && $fileinfo['size']>0){
                ...
                if(!file_exists($path)){
                    ...
                    for($j=2;$j<=$highestRow;$j++){
                        ...
                        $storeTarget=(string)$objPHPExcel->getActiveSheet()->getCell("H$j")->getValue();
                        $hkTarget=(string)$objPHPExcel->getActiveSheet()->getCell("I$j")->getValue();
                        $dateMonth=(string)$objPHPExcel->getActiveSheet()->getCell("J$j")->getValue();
                        
                        $t1 = $dateMonth;
                        $n1 = intval(($t1 - 25569) * 3600 * 24);
                        $dateMonth=gmdate('Y-m',$n1);

                        $sqlstr2="select count(*) from store_target where storeID='$storeID' and dateMonth='$dateMonth'";
