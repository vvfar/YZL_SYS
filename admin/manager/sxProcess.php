...
<html lang="zh-CN">
    ...
    <body>
        ...
        <div style="width: 1660px;height:890px;margin-left: 240px;">
            ...
            <button class="btn btn-success" style="float:left;margin-left:1135px;margin-top:0px;"  data-toggle="modal" data-target="#myModal">添加节点</button>
            
            <div style="clear:both">
                <?php
                    $id=$_GET["id"];
                    $department2=$_GET["department"];
                    $sqlstr1="select distinct department,sxprogress_id from sxprogress where sxprogress_id=$id";
                ...
                <canvas id="myCanvas" width="1300px" height="200" style="clear:both;margin-left:40px;margin-top:0px;border:1px solid #cccccc;">
                    您的浏览器不支持 HTML5 canvas 标签。
                </canvas>
            </div>
            
            <table class="table table-responsive table-bordered table-hover" style="clear:both;position:relative;top: 20px;width: 700px;margin-left: 40px;width:1300px">
                <tr>
                    <th>序号</th>
                    <th>名称</th>
                    <th>审核人</th>
                ...
                <?php
                    ...
                    $sqlstr1="select * from sxprogress where sxprogress_id=$id order by number";
                    ...
                    while($myrow=mysqli_fetch_row($result)){
                        ?>
                            <tr>
                                ...
                                <td><?=$myrow[3]?></td>
                                <td><?=$myrow[4]?></td>
                                <td><a href="updateSXProcess.php?id=<?=$myrow[0]?>">修改</a> | <a href="formHandle/delSXProcess.php?id=<?=$myrow[0]?>&number=<?=$myrow[1]?>&sxprogress_id=<?=$myrow[6]?>&department=<?=$department2?>">删除</a></td>
                            </tr>
                        <?php
                    }
         ...
         <!-- Excel导入模态框 -->
         <form method="POST" action="formHandle/addSXProcess.php" enctype="multipart/form-data">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body" style="height: 320px;">
                            ...
                            <input type="text" class="form-control" name="sxprogress_id" value="<?=$sxprogress_id?>"   style="width:250px;display: none;"/>
                            ...
                            <input type="text" class="form-control" name="no" value=2  style="width:250px;display: none;"/>
...
<script>
    ...
    <?php
        ...
        $sqlstr1="select name from sxprogress where sxprogress_id=$id order by number";
